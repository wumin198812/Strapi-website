...
/**
 * Applies admin-managed redirects (the Strapi `redirect` collection) at request
 * time. Returns a redirect response when the request path matches a published
 * redirect, or null to let the request continue through the proxy chain.
 *
 * Running in the proxy (not `next.config` redirects) means newly created admin
 * redirects take effect without a rebuild — the redirect list is fetched and
 * cached at runtime.
 */
export async function redirectsProxy(
  ...
  try {
    ...
    logNonBlockingError("[redirectsProxy] Redirect lookup failed", error)
