...
function refreshRedirects() {
  redirectFetchPromise ??= fetchRedirects()
    .then(cacheRedirects)
    .catch((error: unknown) => {
      logNonBlockingError("[redirects] Failed to refresh redirect cache", error)
...
export function buildRedirectDestinationUrl(
  ...
  try {
    ...
    logNonBlockingWarning("[redirects] Invalid redirect destination", {
      destination,
    })
  ...
  // External destinations are allowed: editors redirect to docs/community sites.
  // Only same-origin destinations get locale/query normalization below.
